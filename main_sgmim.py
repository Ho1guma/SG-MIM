...
def parse_option():
    parser = argparse.ArgumentParser('SGMIM pre-training script', add_help=False)
    ...
    parser.add_argument('--depth-data-path', type=str, help='path to depth map dataset')
    parser.add_argument('--file-name-path', type=str, help='path to filename txt file')
...
def train_one_epoch(config, model, data_loader, optimizer, epoch, lr_scheduler):
    ...
    for idx, (img, depth, mask) in enumerate(data_loader):
